...
import { Card, FormContainer, SubmitButton, RadioButtonContainer, RowContainer, ModalContent, UpdateContainer } from '../styles/componentStyles';
...
console.log("state value : ",state)
    ...
    const studentCourseRegistrationDetails = useSelector(
        ...
        const getCourseName = (deptName,courseId) =>{
            if(fetchDepartmentsData)
             for(let i=0;i<fetchDepartmentsData[deptName]["courses"].length;i++){
                if(fetchDepartmentsData[deptName]["courses"][i]["_id"] == courseId){
                    return fetchDepartmentsData[deptName]["courses"][i]["title"] 
                }
             }
        }
    useEffect(() => {
    ...
    const handleSubmit = (e) => {
        ...
        if (Object.keys(errors).length > 0) {
            ...
            let duplicateArray = []
            studentCourseRegistrationDetails && studentCourseRegistrationDetails.enrolls.map((obj) => {
                if (obj["courseId"] === courseData.courseName) {
                    duplicateArray.push(`DC`)
                }
                if (obj["day"] === courseData.day && obj["time"] === courseData.time) {
    ...
    return (
        <div>
            ...
            <div style={{display:'flex',margin:'20px'}}>
             { fetchDepartmentsData[state["dept"]] && <Card style={{width:'400px',margin:'20px',height:'200px'}}>
<div style={{padding:'10px'}}>
    
    <div style={{textAlign:"center"}}>
    <span style={{textAlign:'center',fontSize:'20px',fontWeight:'600'}}>Selected values </span>
    </div>
    <UpdateContainer>

    
    <span>Department: </span><span>{state["dept"]}</span>
    </UpdateContainer>
    
    <UpdateContainer>
    <span>Course Name: </span><span>{getCourseName(state["dept"],state["courseId"])}</span>
    </UpdateContainer>
    <UpdateContainer>
    <span>CRN: </span><span>{state["crn"]}</span>
    </UpdateContainer>
    <UpdateContainer>
    <span>Day: </span><span>{state["day"]}</span>
    </UpdateContainer>
    <UpdateContainer>
    <span>Timings: </span><span>{convertTime(state["time"])}</span>
    </UpdateContainer>
    </div>
            ...
            {fetchDepartmentsData &&  <form onSubmit={handleSubmit}>
                    <FormContainer>
                        ...
                        <ButtonContainer>
                            <SubmitButton onClick={()=>handleBack()} type="submit" variant="contained" color="primary">
                                Back
                            </SubmitButton>
                        </ButtonContainer>
                        <ButtonContainer>
...
export default UpdateSpecificCourse;
